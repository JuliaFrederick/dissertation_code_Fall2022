...
# Load Data
vcf_file <- ("./gatk_m90_6x50x_GQ30_Q30_maf05_ma2.recode.vcf.gz")
...
png(file="./reseq_gatkdata_DAPC_populations.png", width = 700, 600)
...
#png(file="./varcontrib_DAPC_gatk.png", width=1000,900)
#contrib.s <- loadingplot(dapc.pop$var.contr, lab.jitter=1)
#dev.off()
...
# Plot DAPC Clusters
png(file="./freseq_gatkdata_DAPC_rmFL097.png", width = 700,600)
...
#png(file="./varcontrib_DAPC_gatk_woFL097.png", width=1000,900)
...
write.csv(sum_stats_pops,"./reseq_gatkdata_sum_stats_pops.csv")

# Pairwise Fst per population
hgi<- genind2hierfstat(gi)

pairnei <- pairwise.neifst(hgi) # Use this data
write.csv(pairnei,file="./reseq_gatkdata_pairnei.csv")
pairwc <- pairwise.WCfst(hgi) # This data is for comparison purposes
write.csv(pairwc,file="./reseq_gatkdata_pairwc.csv")

save.image(file="./DAPC_BStats_reseq_gatk_m90_6x50x_GQ30_Q30_maf05_ma2.RData")
